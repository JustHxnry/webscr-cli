...
async function uploadPdf(b64) {
    ...
    const fileName = randoms.randomString(10) + ".pdf";
...
async function screenshot(url, options, format) {
    ...
    await page.goto(url);
    const pdf64 = await page.pdf({ format });

    await uploadPdf(pdf64);
        ...
        prompt(q).then(width => {
            ...
            prompt(q).then(height => {
                ...
                prompt(q).then(mwv => {
                    ...
                    q = chalk.blue(`Page format (default: A4): `);
                    prompt(q).then(format => {
                        if (!format) format = 'A4';
                        screenshot(url, options, format);
                    });
                });
            });
        });
